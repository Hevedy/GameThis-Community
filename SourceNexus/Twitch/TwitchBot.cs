...
namespace GameThisNexus.Twitch {
	public class TwitchBot {
		...
		public static bool ConfigRead() {
			...
			if ( !data.DataINIReadWrite( "Twitch", "Guild", ref Instance.TwitchGuild ) ) status = false;
			...
			if ( !data.DataINIReadWrite( "Twitch", "BotName", ref Instance.TwitchBotName ) ) status = false;
			if ( !data.DataINIReadWrite( "Twitch", "BotToken", ref Instance.TwitchBotToken ) ) status = false;
			if ( !data.DataINIReadWrite( "Twitch", "UserBlackList", ref Instance.TwitchUserBlackList ) ) status = false;
			if ( !data.DataINIReadWrite( "Twitch", "BotBlackList", ref Instance.TwitchBotBlackList ) ) status = false;
		...
		public static bool ConfigValidate() {
			...
			if ( !HEVText.Validate( Instance.TwitchBotName ) ) return status;
			if ( !HEVText.Validate( Instance.TwitchBotToken ) ) return status;
			status = true;
			return status;
		}
		...
		public static bool ConfigWrite() {
			...
			data["Twitch"]["Guild"] = Instance.TwitchGuild;
			data["Twitch"]["Channel"] = Instance.TwitchChannel;
			data["Twitch"]["UserName"] = Instance.TwitchUserName;
			data["Twitch"]["BotName"] = Instance.TwitchBotName;
			data["Twitch"]["BotToken"] = Instance.TwitchBotToken;

			data["Twitch"]["UserBlackList"] = HEVText.ToString( Instance.TwitchUserBlackList );
			data["Twitch"]["BotBlackList"] = HEVText.ToString( Instance.TwitchBotBlackList );
